...
from .backend import ansible_generation, blueprint_catalog_service, terraform_generation
from .types import BlueprintKind


def list_blueprints(kind: BlueprintKind | None):
    return blueprint_catalog_service.list_blueprints(kind)
...
async def set_active_blueprint(
    ...
    await blueprint_catalog_service.set_active_blueprint(session, project, kind, blueprint_id, inputs)
...
async def create_blueprint_run(
    ...
    return await blueprint_catalog_service.create_blueprint_run(
        session,
        project,
        thread_id,
        kind,
...
async def get_blueprint_run(session: AsyncSession, project_id: str, run_id: str):
    return await blueprint_catalog_service.get_blueprint_run(session, project_id, run_id)
