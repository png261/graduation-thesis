...
function HistoryDialog({ historyOpen, setHistoryOpen }: { historyOpen: boolean; setHistoryOpen: (open: boolean) => void }) {
    return (
...
function SidebarLayout({ children, suggestions, onOpenHistory }: { children: ReactNode; suggestions: Suggestion[]; onOpenHistory: () => void }) {
    return (
            ...
            <ResizablePanel defaultSize={30} minSize={24} className="min-w-0">
                <div className="assistant-chat-pane flex h-full min-h-0 flex-col">
                    <div className="min-h-0 flex-1">
                        <Thread suggestions={suggestions} onOpenHistory={onOpenHistory} />
                    </div>
                </div>
            </ResizablePanel>
...
export function AssistantSidebar({
    ...
    return (
        <section className={cn("assistant-sidebar h-full min-h-0 min-w-0", className)}>
            <SidebarLayout children={children} suggestions={suggestions} onOpenHistory={() => setHistoryOpen(true)} />
